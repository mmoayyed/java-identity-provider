 ...
 * <code>
    <element name="DenyValueRule" type="afp:MatchFunctorType">
        <annotation>
            <documentation>
                A filter for attribute values. If the filter evaluates to true the 
                value is denied and may not be released.
            &lt;/documentation>
        &lt;/annotation>
    &lt;/element>
  </code>
 */
@ThreadSafe
public class DenyValueRule extends AbstractDestructableInitializableComponent implements ValidatableComponent,
        UnmodifiableComponent {

    /** Class logger. */
    private final Logger log = LoggerFactory.getLogger(DenyValueRule.class);

    /**
     * Filter that denies the release of attribute values. Default value: {@link MatchFunctor#MATCHES_ALL}
     */
    private MatchFunctor valueMatchingRule;

    /** Constructor. */
    public DenyValueRule() {
        valueMatchingRule = MatchFunctor.MATCHES_ALL;
    }
