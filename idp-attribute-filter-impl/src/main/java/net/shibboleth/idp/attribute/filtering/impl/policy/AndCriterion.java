 ...
 * 
 * To evaluate this work through the provided in order, following the usual semantics that the first false result will
 * cause us to stop processing. NULL criteria are silently ignored. An entirely empty (or null) list will cause a
 * warning to be emitted, but processing will continue, returning FALSE. This is not perhaps what is expected. but is
 * based a "default deny" approach (just like if (null))
 * 
 */
@ThreadSafe
public class AndCriterion extends AbstractBiasedEvaluableCriterion<AttributeFilterContext> implements
    ...
    public Boolean doEvaluate(final AttributeFilterContext target) throws EvaluationException {
        ...
        if (!isInitialized()) {
            throw new UninitializedComponentException("And Criterion has not been initialized");
        }
