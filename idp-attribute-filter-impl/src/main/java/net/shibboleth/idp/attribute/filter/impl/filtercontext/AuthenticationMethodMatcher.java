 ...
 */
public class AuthenticationMethodMatcher extends AbstractStringMatcher {

    /** The logger. */
    private final Logger log = LoggerFactory.getLogger(AuthenticationMethodMatcher.class);

    /** Constructor. */
    public AuthenticationMethodMatcher() {
    ...
    protected boolean doCompare(@Nullable final AttributeFilterContext filterContext) {
        ...
        if (null == method) {
            throw new MatcherException(getLogPrefix() + " No authetication method found for comparison");
        }
