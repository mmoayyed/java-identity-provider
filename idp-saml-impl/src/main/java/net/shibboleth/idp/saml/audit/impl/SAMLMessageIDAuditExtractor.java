...
/** {@link AuditExtractorFunction} that returns the message ID from a {@link MessageContext}. */
public class SAMLMessageIDAuditExtractor implements AuditExtractorFunction {
     ...
     */
    public SAMLMessageIDAuditExtractor(@Nonnull final Function<ProfileRequestContext,SAMLMessageInfoContext> strategy) {
    ...
    @Nullable public Collection<String> apply(@Nullable final ProfileRequestContext input) {
        ...
        if (infoCtx != null) {
            return Collections.singletonList(infoCtx.getMessageId());
        } else {
