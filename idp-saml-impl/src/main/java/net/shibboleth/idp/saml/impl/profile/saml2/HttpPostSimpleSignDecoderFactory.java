...
import org.opensaml.saml.saml2.binding.decoding.HTTPPostSimpleSignDecoder;

/** Factory used to produce {@link HTTPPostSimpleSignDecoder} message decoders. */
@ThreadSafe
public class HttpPostSimpleSignDecoderFactory implements HttpServletRequestMessageDecoderFactory {
     ...
     */
    public HttpPostSimpleSignDecoderFactory(@Nonnull final ParserPool pool) {
    ...
    public MessageDecoder newDecoder(@Nonnull final HttpServletRequest httpRequest) throws MessageDecodingException {
        HTTPPostSimpleSignDecoder decoder = new HTTPPostSimpleSignDecoder();
