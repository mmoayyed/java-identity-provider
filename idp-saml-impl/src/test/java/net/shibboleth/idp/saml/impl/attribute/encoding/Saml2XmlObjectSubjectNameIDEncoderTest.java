...
public class Saml2XmlObjectSubjectNameIDEncoderTest extends OpenSAMLInitBaseTestCase {
    ...
    @Test public void single() throws Exception {
        final Collection<? extends AttributeValue<?>> values =
                Lists.newArrayList(saml1NameIdFor(OTHERID), new StringAttributeValue("foo"), saml2NameIdFor(NAME_1),
                        saml1NameIdFor(NAME_2));
        final IdPAttribute inputAttribute = new IdPAttribute(ATTR_NAME);
        inputAttribute.setValues(values);

        final NameID outputNameId = encoder.encode(inputAttribute);
    ...
    @Test public void multi() throws Exception {
        final Collection<? extends AttributeValue<?>> values =
                Lists.newArrayList(saml1NameIdFor(OTHERID), saml2NameIdFor(NAME_1), saml2NameIdFor(NAME_1));
        ...
        Assert.assertNotNull(outputNameId);
        Assert.assertEquals(outputNameId.getNameQualifier(), QUALIFIER);
        String nameName = outputNameId.getValue();
